...
- `src/linker/*.lds` are preprocessed with the C preprocessor (`cmake/linker_script.cmake`, `-x assembler-with-cpp`) before being handed to `ld`, so they can `#include` headers and use `#define` constants — see `src/platform/boot/include/platform/stacks.h` for the mode-stack size/color macros shared with `boot_reset.S`.
- Linker-provided region-boundary symbols follow `_{region}_begin`/`_{region}_end` (e.g. `_stack_begin`/`_stack_end`, `_empty_ram_begin`/`_empty_ram_end`). Don't reuse a narrower symbol as a stand-in for a wider region's boundary just because the addresses currently coincide (e.g. `_stack_unused` bounds only the ABT/UND stack, not the whole stack area).
