...
int command_efuse(__unused int argc, __unused const char *argv[]) {
    uint8_t buf[32] = {0};
    ...
    if (efuse_read_charge_cal(buf) == 0) {
        printf("efuse_read_charge_cal: %02x %02x %02x %02x\r\n", buf[0], buf[1], buf[2], buf[3]);
    } else {
        printf("efuse_read_charge_cal: error\r\n");
    }

    if (efuse_read_uid(buf) == 0) {
        printf("efuse_read_uid: %02x %02x %02x %02x\r\n", buf[0], buf[1], buf[2], buf[3]);
    } else {
        printf("efuse_read_uid: error\r\n");
    }

    if (efuse_read_mac(buf) == 0) {
        printf("efuse_read_mac: %02x %02x %02x %02x %02x %02x\r\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
    } else {
