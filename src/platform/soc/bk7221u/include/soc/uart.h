...
typedef volatile struct {
    union {
        ...
        struct {
            ...
            uint32_t irda : 1;            // [2]
            uint32_t data_length : 2;     // [4:3]  0=5bit 1=6bit 2=7bit 3=8bit
    ...
    union {
        uint32_t v;
        struct {
            uint32_t tx_fifo_threshold : 8;   // [7:0]
            uint32_t rx_fifo_threshold : 8;   // [15:8]
            uint32_t rx_stop_detect_time : 2; // [17:16]
    ...
    union {
        uint32_t v;
        struct {
            uint32_t low_cnt : 8;         // [7:0]
            uint32_t high_cnt : 8;        // [15:8]
    ...
    union {
        uint32_t v;
        struct {
            uint32_t wake_count : 10;             // [9:0]
            uint32_t txd_wait_count : 10;         // [19:10]
