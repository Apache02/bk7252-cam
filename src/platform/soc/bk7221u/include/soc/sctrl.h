...
typedef volatile struct {
    ...
    union {
        ...
        struct {
            ...
            uint32_t period: 16;       // [31:16] low 16 bits of timer period in ROSC cycles;
                                       // high 16 in rosc_timer_h.period_h
        };
    } rosc_timer;
    ...
    union {
        uint32_t v;
        struct {
            uint32_t value: 16; // [15:0]  scratch; survives sleep and soft reset (jump to 0x0); cleared by
                                // WDT/power-on reset. On BK7221U the SDK uses 0x0080A080 (JPEG quant-table RAM) for
                                // start-type detection — that address survives WDT reset, this one does not
...
__unused static inline void sctrl_block_enable_temprature_sensor() {
    hw_write_fields(hw_sctrl->block_enable, .temprature_sensor = 1, .write_key = SCTRL_BLOCK_ENABLE_WRITE_KEY, );
}

__unused static inline void sctrl_block_disable_temprature_sensor() {
    hw_write_fields(hw_sctrl->block_enable, .temprature_sensor = 0, .write_key = SCTRL_BLOCK_ENABLE_WRITE_KEY, );
}
